...
# }
#
define dspace::install ($owner             = $username,
                        $group             = $owner,
...

{
  ...
  -> file { $src_dir:
    ...
    -> exec { "Checkout branch ${git_branch}" :
       ...
       user    => $owner,
       path    => "/home/${owner}/dspace-src",
# Only perform this checkout if the branch EXISTS and it's NOT currently checked out
# (if checked out it will have '*' next to it in the branch listing)
  ...
  file { "${src_dir}/dspace/config/local.cfg":
   ...
   exec { "Build DSpace installer in ${src_dir}":
     ...
     cwd         => $src_dir, # Run command from this directory
     user        => $owner,
     path        => '/dspace-source/dspace/target/dspace-installer',
